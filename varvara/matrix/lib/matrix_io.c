...
int InitMatrix(Matrix_NxM *matrix)
{
    ...
    {
        perror("Не удалось выделить память для динамической матрицы!\n");
        return 0;
    }
...
int freeMatrix(Matrix_NxM *matrix)
{
    if (matrix == NULL)
    {
        fprintf(stderr, "Матрица не инициализирована!\n");
        return 0;
    }
...
int searchingForElementIndexDynamicMatrix(const Matrix_NxM *matrix, int row, int column)
{
    ...
    {
        fprintf(stderr, "Некорректные индексы элемента в матрице!\n");
        return 0;
    }
...
int readingMatrixFromFile(Matrix_NxM *matrix, const char *pathToFile)
{
    ...
    {
        fprintf(stderr, "Матрица или путь к файлу не инициализированы!\n");
        return 0;
    }
    ...
    {
        fprintf(stderr, "Неверный формат файла - не удалось прочитать размеры матрицы!\n");
        fclose(file);
        return 0;
    }
...
Matrix_NxM createMatrix(int rows, int columns)
{
    ...
    if (rows <= 0 || columns <= 0)
    {
        fprintf(stderr, "Неверные значения для количесвта строк и столбцов\n.");
        return result;
    }
    ...
    {
        fprintf(stderr, "Функция int InitMatrix(Matrix_NxM *matrix) вернула ошибку!\n.");
        return result;
    }
