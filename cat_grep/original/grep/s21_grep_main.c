...
int main(int argc, char *argv[]) {
  ...
  int file_start_index = parse_options(argc, argv, &flags, &pattern,
                                       &pattern_dynamic, &program_execution);
  ...
  if (!program_execution && !(flags & FLAG_P)) {
    ...
    if (file_start_index == argc) {
      process_file_grep(flags, pattern, stdin, "-", file_count,
                        &program_execution);
    } else {
      for (int i = file_start_index; i < argc; i++) {
        ...
        process_file_grep(flags, pattern, file, argv[i], file_count,
                          &program_execution);
